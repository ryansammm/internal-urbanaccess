...
$(document).on('click', '.vendor', function () {
    ...
    $.ajax({
        ...
        for (let index = 0; index < data.length; index++) {
            const element = data[index];
            listVendor += '<div class="col-md-6"><div class="card-body form-group border border-1 rounded"><h5>' + element.namaVendor + '</h5><div class="row"><div class="col-md-6"><label for="first-name-vertical">Tanggal Keluar Hasil Survey</label><input type="date" class="form-control" name="tanggalHasil-' + element.idVendor + '" required></div><div class="col-md-6"><label for="first-name-vertical">Status</label><select name="hasil-' + element.idVendor + '" id="" class="form-select" required><option value="1">Tercover</option><option value="2">Tidak Tercover</option></select></div></div><div class="row"><div class="col-md-6"><label for="first-name-vertical">Jarak</label><div class="input-group "><input type="text" class="form-control" aria-label="Amount (to the nearest dollar)" name="jarak-' + element.idVendor + '" required><span class="input-group-text">m</span></div></div><div class="col-md-6"><label for="first-name-vertical">Biaya Instalasi</label><div class="input-group"><span class="input-group-text">Rp.</span><input type="text" class="form-control number" name="biayaInstalasi-' + element.idVendor + '" required></div></div></div><div class="row"><div class="col-md-12"><label for="first-name-vertical">Keterangan</label><textarea name="keterangan-' + element.idVendor + '" id="" class=" form-control"></textarea></div></div></div></div>';
