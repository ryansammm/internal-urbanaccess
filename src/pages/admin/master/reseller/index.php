        ...
        <section class="section">
            <div class="card">
                <div class="card-content">
                    <div class="card-body">
                        <h4 class="card-title mb-3">Daftar Sales Korporasi</h4>
                        <div style="background-color: #589cd1;height: 2px;margin-bottom: 10px;"></div>
                        <a type="button" class="btn btn-sm btn-primary mb-1" href="/reseller/create">Tambah Data <i class="fas fa-plus"></i></a>
                        <table class="table" id="table1">
                            ...
                            <tbody>
                                <?php foreach ($datas as $key => $value) { ?>
                                    <tr>
                                        <td><?= $key += 1 ?></td>
                                        <td><?= $value['nikSales'] ?></td>
                                        <td><?= $value['namaSales'] ?></td>
                                        <td><?= $value['singkatanSales'] ?></td>
                                        <td>Aktif</td>
                                        <td>
                                            <a href="/reseller/<?= $value['idSales'] ?>/edit" class="btn btn-sm btn-outline-primary"><i class="fas fa-edit"></i></a>
                                            <a href="/reseller/detail/<?= $value['idSales'] ?>" class="btn btn-sm btn-outline-warning"><i class="fas fa-eye"></i></a>
...
<script>
    ...
    $(document).on('click', '.btn-hapus', function() {
        ...
        modal.find('.form-hapus').prop('action', '/reseller/' + id + '/delete');
    })
